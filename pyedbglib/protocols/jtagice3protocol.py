...
class Jtagice3Command(AvrCommand):
    ...
    def validate_response(self, response):
        ...
        # Check length first
        if len(response) < 5:
            raise Exception("Invalid response length ({:d}).".format(len(response)))

        # Check token
        if response[0] != self.JTAGICE3_TOKEN:
            raise Exception("Invalid token (0x{:02X}) in response.".format(response[0]))

        # Check sequence
        sequence = response[1] + (response[2] << 8)
        if self.sequence_id != sequence:
            raise Exception("Invalid sequence in response (0x{:04X} vs 0x{:04X}).".format(self.sequence_id, sequence))

            # Check handler
        if response[3] != self.handler:
            raise Exception("Invalid handler (0x{:02X}) in response.".format(response[3]))
